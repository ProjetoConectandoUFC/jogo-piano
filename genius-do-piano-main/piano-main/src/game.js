...
// Funções de utilidade para manipulação de sons
export const States = {
  IDLE: 'idle',
  INTRO: 'intro',
  COUNTDOWN: 'countdown',
...
// Função para permitir que observadores se inscrevam em eventos específicos
export function on(event, handler) { listeners[event].push(handler); }
// Função para emitir eventos para todos os observadores inscritos
function emit(event, payload) { for (const h of listeners[event]) h(payload); }
// Estado atual do jogo, melodia em andamento, nível, notas esperadas e recebidas
...
// A primeira nota do jogador define o offset (em semitons) para todas as notas subsequentes.
function pitchClass(midi) {
  return ((midi % 12) + 12) % 12;
}
// Função auxiliar para criar uma pausa assíncrona, usada para cadenciar a execução
const sleep = (ms) => new Promise((r) => setTimeout(r, ms));
// Função para obter o estado atual do jogo, permitindo que observadores externos consultem
export function getState() { return currentState; }
// Função para alterar o estado do jogo, registrando informações adicionais e notificando observadores
...
// Função para limpar o watchdog, cancelando qualquer timeout pendente
function clearWatchdog() {
  if (watchdog) { clearTimeout(watchdog); watchdog = null; }
}
// Função para armar o watchdog, configurando um timeout que dispara a derrota se o jogador demorar demais
function armWatchdog(ms) {
  clearWatchdog();
  watchdog = setTimeout(() => finishLose('Tempo esgotado.'), ms);
}
// Função para calcular os tempos de início esperados para cada nota da melodia, com base no BPM e na duração das notas
...
//  Função para tocar a demonstração da melodia, respeitando o ritmo e a cadência, e permitindo que o jogador veja e ouça antes de tentar reproduzir
...
// Função para iniciar uma nova rodada do jogo, configurando a melodia, nível e estado inicial, e conduzindo o jogador através da introdução, contagem regressiva, demonstração e entrada de notas
...
// Função para calcular a pontuação de ritmo com base na consistência dos intervalos entre as notas tocadas pelo jogador em relação aos intervalos esperados da melodia
...
// Função para determinar a classificação do jogador com base na pontuação de ritmo, retornando rótulo, número de estrelas e mensagem adicional
...
// Função para finalizar a rodada com vitória, calculando a pontuação de ritmo e determinando a classificação do jogador
function finishWin() {
  clearWatchdog();
...
// Função para finalizar a rodada com derrota, registrando o motivo e o nível atual
function finishLose(reason) {
  clearWatchdog();
  setState(States.LOSE, { reason, level: currentLevel });
}
// Função para lidar com a entrada de notas do jogador, validando pitch e ritmo, e determinando se a rodada continua ou termina com vitória ou derrota
...
// Função para abortar a rodada atual, cancelando qualquer fluxo assíncrono em andamento e retornando ao estado IDLE
